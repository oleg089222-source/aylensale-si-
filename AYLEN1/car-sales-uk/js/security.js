...
var SECURITY = {
  ...
  checkFormRateLimit: function() {
    ...
    localStorage.setItem('aylen_form_attempts', JSON.stringify(this.formAttempts));
  ...
  checkNamedRateLimit: function(field, storageKey, maxAttempts, windowMs, reason) {
    ...
    this[field] = (this[field] || []).filter(function(time) {
      return time > cutoff;
    });
    if (this[field].length >= maxAttempts) {
      return { allowed: false, reason: reason, remaining: 0 };
    }
    this[field].push(now);
    try {
    ...
    return { allowed: true, remaining: maxAttempts - this[field].length };
  },

  validateHoneypot: function(value) {
    return value ? { valid: false, error: 'Submission blocked.' } : { valid: true };
  },

  validateMinimumSubmitTime: function(startedAt, minimumMs) {
    var start = Number(startedAt || 0);
    var minimum = Number(minimumMs || 1200);
    if (start && Date.now() - start < minimum) {
      return { valid: false, error: 'Please wait a moment before submitting.' };
    }
    return { valid: true };
  },

  submissionMeta: function(startedAt, honeypotValue) {
    return {
      sessionId: this.sessionId,
      formStartedAt: Number(startedAt || Date.now()),
      submittedAt: Date.now(),
      website: honeypotValue || ''
    };
  },

  validateBidForm: function(name, phone, contact, honeypotValue, startedAt) {
    var hp = this.validateHoneypot(honeypotValue);
    if (!hp.valid) return hp;

    var timing = this.validateMinimumSubmitTime(startedAt, 1000);
    ...
    var commentValidation = this.validateComment(contact);
    if (!commentValidation.valid) return commentValidation;

    var botCheck = this.detectBot(name, '', phone, contact);
  ...
  protectContent: function() {
    document.addEventListener('contextmenu', function(e) {
      if (e.target && (e.target.tagName === 'IMG' || e.target.closest('.product-card') || e.target.closest('.auction-card'))) {
        e.preventDefault();
      }
    });
  ...
  init: function() {
    try {
      ...
      var storedForms = localStorage.getItem('aylen_form_attempts');
      if (storedForms) this.formAttempts = JSON.parse(storedForms);
      var storedBids = localStorage.getItem('aylen_bid_attempts');
      if (storedBids) this.bidAttempts = JSON.parse(storedBids);
      var storedWinners = localStorage.getItem('aylen_winner_attempts');
      if (storedWinners) this.winnerAttempts = JSON.parse(storedWinners);
    } catch (e) {
