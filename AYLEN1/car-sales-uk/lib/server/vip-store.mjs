...
function isAuctionOpenForBids(data) {
  ...
  return status !== 'completed' && status !== 'order_sent' &&
    status !== 'winner_pending' && status !== 'paid' &&
...
export function hasVipAccess(record) {
  ...
  if (status === VIP_STATUSES.ACTIVE || status === VIP_STATUSES.CANCELLED) {
    ...
    if (status === VIP_STATUSES.ACTIVE) {
      return !!String(record.stripeSubscriptionId || '').trim();
    }
  }
...
export async function listVipAuctionsPreview(limit) {
  ...
  return snap.docs.map(function(doc) {
    ...
    const vipEarlyAccess = earlyMod.isVipEarlyAccessActive(d);
    return {
      id: doc.id,
      ...
      publicStartAt: d.publicStartAt || null,
      vipEarlyAccessHours: Number(d.vipEarlyAccessHours || 0),
      vipEarlyAccess: vipEarlyAccess,
...
export async function placeVipAuctionBid(params) {
  ...
  let bidderKey = String(params && params.bidderKey || '').trim();
  if (!bidderKey && phone) {
    const engine = await import('./auction-engine.mjs');
    bidderKey = engine.phoneKey(phone);
  }
  if (!bidderKey) {
    bidderKey = normalizeBidderKey({ contact: contact, email: contact, phone: phone, name: name });
  }
  const vipMemberId = String(params && params.vipMemberId || '').trim();
  const skipAntiSpam = !!(params && params.skipAntiSpam);
  const skipDeposit = !!(params && params.skipDeposit);
  ...
  if (!preSnap.exists) throw new Error('Auction not found');
  const preData = preSnap.data() || {};

  if (!skipDeposit) {
    const depositMod = await import('./auction-deposit.mjs');
    const engine = await import('./auction-engine.mjs');
    const settings = await engine.getAuctionSettings(db);
    const depositFlags = depositMod.resolveDepositFlags(settings);
    try {
      await depositMod.requirePaidDepositForBid(db, preData.deposits, bidderKey, depositFlags);
    } catch (depErr) {
      if (depErr && depErr.code === 'DEPOSIT_REQUIRED') {
        const err = new Error(depErr.message || 'Deposit required before bidding');
        err.code = 'DEPOSIT_REQUIRED';
        err.depositAmountGbp = depErr.depositAmountGbp;
        throw err;
      }
      throw depErr;
    }
  ...
  const result = await db.runTransaction(async function(tx) {
    ...
    bids.push(mapVipAuctionBidPublic(bid));
    tx.set(ref, {
      bids: bids,
  ...
  try {
    ...
    const bidRecord = Object.assign({}, result.bid || {}, {
      isBot: false,
      source: String(params && params.source || 'vip'),
      bidderPhone: phone,
      bidderKey: bidderKey
    });
    await engine.afterRealBid(db, auctionId, bidRecord, Object.assign({ id: auctionId }, postSnap.data() || {}));
  } catch (hookErr) {
    console.warn('[vip-auction-bid] afterRealBid:', hookErr.message || hookErr);
  }
