...
export async function saveShopOrderToFirestore(order) {
  ...
  const data = {
    ...
    stockReservationId: order.stockReservationId ? String(order.stockReservationId).slice(0, 120) : '',
    stockAdjustments: Array.isArray(order.stockAdjustments)
      ? order.stockAdjustments.slice(0, 50).map(function(adj) {
          return {
            productId: String(adj.productId || '').slice(0, 120),
            name: String(adj.name || '').slice(0, 140),
            before: Number(adj.before || 0),
            after: Number(adj.after || 0),
            qty: Number(adj.qty || 0),
            vipSynced: Number(adj.vipSynced || 0)
          };
        })
      : [],
