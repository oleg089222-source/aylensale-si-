/**
 * GET /api/manifest — dynamic PWA manifest (Firestore branding icons + static fallback).
 */
import { loadBrandingDoc } from './lib/branding-store.mjs';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.setHeader('Content-Type', 'application/manifest+json; charset=utf-8');
    return res.status(200).json(Object.assign({}, STATIC_MANIFEST, {
      icons: [
        iconEntry('/icon-192.png', '192x192', 'any'),
        iconEntry('/icon-512.png', '512x512', 'any'),
        iconEntry('/maskable-192.png', '192x192', 'maskable'),
