/**
 * Admin-only bulk storage resize API (runs on Vercel with production Firebase creds).
 */
...
import { fmtBytes, runStorageBulkResize } from './storage-bulk-resize.mjs';
...
export async function handleStorageResize(req, res) {
  cors(res);
  ...
  try {
    const result = await runStorageBulkResize({ apply, limit });
    const summaryRows = (result.rows || []).slice(0, 40).map(function(row) {
      if (row.error) return { name: row.name, error: row.error };
      return {
        name: row.name,
        before: row.before,
        after: row.after,
        saved: row.before && row.after ? fmtBytes(row.before - row.after) : undefined,
        resized: row.resized
      };
    });
    return res.status(200).json({
      ...
      plannedOrApplied: result.planned,
    ...
    console.error('[storage-resize]', err);
